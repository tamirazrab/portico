...
export async function createorupdatecron(data: { cronExpression: string; credentialId: string; workflowId: string; userId: string; timezone?: string }, id?: string) {
...
export async function removeCron(data: { credentialId: string; workflowId: string; userId: string }) {
    try {
        const credentialResult = await getCredentialUseCase({
            id: data.credentialId,
            userId: data.userId,
